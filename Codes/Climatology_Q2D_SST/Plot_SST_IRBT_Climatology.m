...
%% Load ERA5 for cloud cover threshold:

load /Users/yuhungjui/Research_tmp/2021_Q2D/DATA_ERA5/monthly_CLOUD_1998_2020_40SN.mat;

ERA5_variable = ERA5_data.tcc.*100;
...
for ssti = 1:numel(SST_range)-1
    ...
    IRBT_range_mean(ssti) = nanmean(data_IRBTi(SST_range_id));
    IRBT_range_std(ssti) = nanstd(data_IRBTi(SST_range_id));
    IRBT_range_num(ssti) = sum(~isnan(data_IRBTi(SST_range_id)));
...
f11 = line(SST_ticks,IRBT_range_mean);
...
% f00 = line(SST_ticks,SST_ticks+273.15);
% 
% f00.Color = [0.2,0.2,0.2];
% f00.LineStyle = '--';
% f00.MarkerSize = 16;
% f00.LineWidth = 1.5;
% 
...
f12 = line(SST_ticks,IRBT_range_num);
