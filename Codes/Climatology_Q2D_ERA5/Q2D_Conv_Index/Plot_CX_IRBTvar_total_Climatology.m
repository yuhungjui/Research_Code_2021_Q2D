...
Var_Map_total = GRIDSAT_IRBT_VAR.total;
...
for ti = 1:numel(date_dur)
    Var_Map_total_ocean(:,:,ti) = Var_Map_total(:,:,ti).*oceanid';
    Var_Map_total_land(:,:,ti) = Var_Map_total(:,:,ti).*landid';
end

% Var_Map_total = Var_Map_total_ocean;
...
for date_i = 1:numel(date_dur)
    ...
    data_IRBTvari_total_ocean(:,:,date_i) = interp2(data_IRBT_lon,data_IRBT_lat,Var_Map_total_ocean(:,:,date_i)',LONi,LATi)';
    data_IRBTvari_total_land(:,:,date_i) = interp2(data_IRBT_lon,data_IRBT_lat,Var_Map_total_land(:,:,date_i)',LONi,LATi)';
...
if ( cloud_frac_switch == 1 )
    ...
    data_IRBTvari_total_ocean( data_ISCCPi < 40 ) = nan;
    data_IRBTvari_total_land( data_ISCCPi < 40 ) = nan;
end
...
for ssti = 1:numel(SFC_range)-1
    ...
    IRBTvar_total_range_mean(ssti) = nanmean(data_IRBTvari_total(SFC_range_id));
    IRBTvar_total_range_mean_ocean(ssti) = nanmean(data_IRBTvari_total_ocean(SFC_range_id));
    IRBTvar_total_range_mean_land(ssti) = nanmean(data_IRBTvari_total_land(SFC_range_id));
    IRBTvar_total_range_std(ssti) = nanstd(data_IRBTvari_total(SFC_range_id));
    IRBTvar_total_range_num(ssti) = sum(~isnan(data_IRBTvari_total(SFC_range_id)));
    IRBTvar_total_range_num_ocean(ssti) = sum(~isnan(data_IRBTvari_total_ocean(SFC_range_id)));
    IRBTvar_total_range_num_land(ssti) = sum(~isnan(data_IRBTvari_total_land(SFC_range_id)));
    
    SST_range_num(ssti) = numel(SFC_range_id);
...
IRBTvar_total_range_mean_ocean(IRBTvar_total_range_num_ocean<data_num_cri) = NaN;
IRBTvar_total_range_mean_land(IRBTvar_total_range_num_land<data_num_cri) = NaN;
...
f11 = line(SFC_ticks,IRBTvar_total_range_mean);
...
f11_o = line(SFC_ticks,IRBTvar_total_range_mean_ocean);
...
f11_l = line(SFC_ticks,IRBTvar_total_range_mean_land);
...
f21 = line(SFC_ticks,IRBTvar_total_range_num);
...
leg1 = legend([f11,f11_o,f11_l,f21],{'Total var.','Total var. (ocean)','Total var. (land)','num.'});
...
figname = ['./',ERA5_CX_var_name,'_IRBTvar_Climatology'];
